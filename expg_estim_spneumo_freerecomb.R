...
nsims <- 75000 #number of sims
#' get parameters as observed from the data
load(paste0("data/pneu_syn_obsdiv",winsize/1000,"K.RData"))



svec <- obs_stats$S[2:16]
#first entry is genomewide, we want only 250 kb windows
#and S as well as coverage breaks down after ~950 kb 
#we are keeping the first 15 windows 
...
nsfs <- read.delim("data/MinAlpha-2-MaxAlpha-2-NoStepAlpha-0-MinRho-0-MaxRho-25-NoStepRho-250-noSamples-315_expectedPhi.txt", header=FALSE)
...
save(sims1,g_params,drawS,
     file = paste0("sims_pneumo_PRF",coh,"_",winsize/1000,"K.RData"),compress = "xz")
}
...
load(paste0("sims_pneumo_PRF",coh,"_",winsize/1000,"K.RData"))
sims2 <- t(sims1)
g_params2 <- g_params
...
save(growthfit_rf,growth_rf,file=paste0("res_100winspneumoPRF_",winsize/1000,"K.RData"),compress = "xz")
}
